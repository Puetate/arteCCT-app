import 'package:arte_ctt_app/src/domain/models/picture.dart';

final pictures = [
  const Picture(
      id: '1',
      name: 'Flores',
      autor: 'elpepe',
      urlImage:
          'https://culturatungurahua.com/wp-content/uploads/2022/12/cuadro-24.gif',
      bibliography:
          'Marilyn Manson, cuyo nombre real es Brian Hugh Warner, nació el 5 de enero de 1969 en Canton, Ohio, Estados Unidos. Es un músico, cantante y actor reconocido por su estilo controvertido y provocador.',
      description:
          'La Mona Lisa, también conocida como La Gioconda, es una famosa pintura creada por el renombrado artista italiano Leonardo da Vinci. La obra maestra es un retrato en óleo sobre tabla de madera de una mujer con una enigmática sonrisa.'),
  const Picture(
      id: '2',
      name: 'Marina',
      autor: 'elalex',
      urlImage:
          'https://culturatungurahua.com/wp-content/uploads/2022/12/cuadro-25.gif',
      bibliography:
          'Marilyn Manson, cuyo nombre real es Brian Hugh Warner, nació el 5 de enero de 1969 en Canton, Ohio, Estados Unidos. Es un músico, cantante y actor reconocido por su estilo controvertido y provocador.',
      description:
          'La Mona Lisa, también conocida como La Gioconda, es una famosa pintura creada por el renombrado artista italiano Leonardo da Vinci. La obra maestra es un retrato en óleo sobre tabla de madera de una mujer con una enigmática sonrisa.'),
  const Picture(
      id: '3',
      name: 'Caballos Briosos',
      autor: 'lachina',
      urlImage:
          'https://culturatungurahua.com/wp-content/uploads/2022/12/cuadro-21.gif',
      bibliography:
          'Marilyn Manson, cuyo nombre real es Brian Hugh Warner, nació el 5 de enero de 1969 en Canton, Ohio, Estados Unidos. Es un músico, cantante y actor reconocido por su estilo controvertido y provocador.',
      description:
          'La Mona Lisa, también conocida como La Gioconda, es una famosa pintura creada por el renombrado artista italiano Leonardo da Vinci. La obra maestra es un retrato en óleo sobre tabla de madera de una mujer con una enigmática sonrisa.'),
  const Picture(
      id: '4',
      name: 'Mi Mejor Amigo',
      autor: 'yo',
      urlImage:
          'https://culturatungurahua.com/wp-content/uploads/2022/12/cuadro-15.gif',
      bibliography:
          'Marilyn Manson, cuyo nombre real es Brian Hugh Warner, nació el 5 de enero de 1969 en Canton, Ohio, Estados Unidos. Es un músico, cantante y actor reconocido por su estilo controvertido y provocador.',
      description:
          'La Mona Lisa, también conocida como La Gioconda, es una famosa pintura creada por el renombrado artista italiano Leonardo da Vinci. La obra maestra es un retrato en óleo sobre tabla de madera de una mujer con una enigmática sonrisa.'),
  const Picture(
      id: '5',
      name: 'Noche Azul',
      autor: 'TIgiselma',
      urlImage:
          'https://culturatungurahua.com/wp-content/uploads/2022/12/cuadro-29.gif',
      bibliography:
          'Marilyn Manson, cuyo nombre real es Brian Hugh Warner, nació el 5 de enero de 1969 en Canton, Ohio, Estados Unidos. Es un músico, cantante y actor reconocido por su estilo controvertido y provocador.',
      description:
          'La Mona Lisa, también conocida como La Gioconda, es una famosa pintura creada por el renombrado artista italiano Leonardo da Vinci. La obra maestra es un retrato en óleo sobre tabla de madera de una mujer con una enigmática sonrisa.'),
  const Picture(
      id: '6',
      name: 'Muerte',
      autor: 'elpepe',
      urlImage:
          'https://culturatungurahua.com/wp-content/uploads/2022/12/cuadro-16.gif',
      bibliography:
          'Marilyn Manson, cuyo nombre real es Brian Hugh Warner, nació el 5 de enero de 1969 en Canton, Ohio, Estados Unidos. Es un músico, cantante y actor reconocido por su estilo controvertido y provocador.',
      description:
          'La Mona Lisa, también conocida como La Gioconda, es una famosa pintura creada por el renombrado artista italiano Leonardo da Vinci. La obra maestra es un retrato en óleo sobre tabla de madera de una mujer con una enigmática sonrisa.'),
];
